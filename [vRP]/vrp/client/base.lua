...
function tvRP.notify(msg, nType, nTitle, nMsec)
  ...
  TriggerEvent("toasty:Notify", {type = "info", title = nTitle or "INFO", message = msg})
...
local wTime = 600 	
    while true do 		
        if IsControlPressed(1, 164) then
            wTime = 1 		
...
Citizen.CreateThread(function()
	while true do
		...
		if(IsControlPressed(0, 249))then
			if(hasMouthCovered == true)then
				NetworkSetVoiceChannel(95)
				NetworkSetVoiceActive(true)
				NetworkSetTalkerProximity(1.0)
				if(isPlayingSound == false)then
					isPlayingSound = true
					TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 5.0, 'gag', 0.2)
				end
			end
		elseif(IsControlReleased(0, 249))then
			if(hasMouthCovered == true)then
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(3600000) -- odata la jumatate de ora se adauga cate 0.50h in scoreboard, daca folositi scoreboard ca pe Wattz.
		vRPserver.updateHoursPlayed({1})
	end
end)
