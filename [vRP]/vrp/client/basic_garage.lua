...
function tvRP.spawnGarageVehicle(vtype,name,veh_plate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
  ...
  if vehicle and not IsVehicleDriveable(vehicle[3]) then -- precheck if vehicle is undriveable
	...
	if vehicle == nil then
		...
		if HasModelLoaded(mhash) then
			...
			TriggerServerEvent('esk:getTuning',name)
		...
		tvRP.notify("S-a despawn masina veche, reincearca!")
		tvRP.despawnGarageVehicle(vtype,99999999999)
	end
end
...
function tvRP.getNearestOwnedVehicle(radius)
  ...
  for k,v in pairs(vehicles) do
    ...
    if dist <= radius+0.0001 then return true,v[1],v[2],v[3] end
  end
...
function tvRP.vc_toggleLock(vtype)
  ...
  --  TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10.0, "carLock", 0.8)
    ...
    if locked then
      ...
      tvRP.notify("Masina descuiata")
    else
      SetVehicleDoorsLocked(veh,2)
      SetVehicleDoorsLockedForAllPlayers(veh, true)
      tvRP.notify("Masina incuiata")
    end
  end
end
