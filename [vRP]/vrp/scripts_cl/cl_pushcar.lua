...
Citizen.CreateThread(function()
    local waititme = 1000
    while true do 
        Citizen.Wait(waititme)
        local ped = PlayerPedId()
        if Vehicle.Vehicle ~= nil then
                waititme = 0
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Apasa [~g~SHIFT~w~] + [~g~E~w~] pentru a impinge masina', 0.4)
                end
