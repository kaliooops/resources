...
function vRP.sendServiceAlert(sender, service_name,x,y,z,msg)
  ...
  if service then
    ...
    for k,v in pairs(vRP.rusers) do
      ...
      -- check user
		if(service_name == "Politie") and player ~= nil then
      if(vRP.isUserInFaction(tonumber(k), "Politia Romana")) then
				table.insert(players,player)
			end
		elseif(service_name == "hitman") and player ~= nil then
			if(vRP.isUserInFaction(tonumber(k), "Hitman"))then
				table.insert(players,player)
			end
		elseif(service_name == "Smurd") and player ~= nil then
			if(vRP.isUserInFaction(tonumber(k), "Smurd"))then
				table.insert(players,player)
			end
		elseif(service_name == "Taxi") and player ~= nil then
			if(vRP.isUserInFaction(tonumber(k), "Taxi"))then
				table.insert(players,player)
			end
		elseif(service_name == "tractari") and player ~= nil then
			if(vRP.isUserInFaction(tonumber(k), "Tractari Auto"))then
				table.insert(players,player)
			end
		else
...
local advert_menu = {name=lang.phone.title(),css={top="75px",header_color="rgba(0,125,255,0.75)"}}
...
-- nest menu
announce_menu.onclose = function(player) vRP.closeMenu(player) end
...
CreateThread(function()
  while true do
    Wait(5000) 
    for k,v in pairs(p_cooldown) do
      p_cooldown[k] = v-5
      if p_cooldown[k] <= 0 then
        p_cooldown[k] = 0
      end
    end
  end
...
local function ch_announce_alert(player,choice) -- alert a announce
  ...
  if p_cooldown[user_id] == nil then 
    p_cooldown[user_id] = 0 
  end

  if p_cooldown[user_id] > 0 then
    TriggerClientEvent("toasty:Notify", player, {type="error", title='[CNN]', message="Asteapta pentru inca ".. p_cooldown[user_id] .." secunde."})
    return
  else
    p_cooldown[user_id] = 15 
  end
      ...
      vRP.prompt(player,lang.phone.announce.prompt(),"",function(player, msg)
        ...
        if string.len(msg) > 10 and string.len(msg) < 1000 then
          if announce.price <= 0 or vRP.tryPayment(user_id, announce.price) then -- try to pay the announce
            ...
            msg = string.gsub(""..GetPlayerName(player).."(" .. user_id .. ")" .. " : "..msg, "\n", "<br />") -- allow returns

            TriggerEvent("Achievements:UP_Current_Progress", user_id, "Pune un anuntat la CNN.")
...
local function build_news_checkpoint(source)
    local function advert_enter(source,area)
		local user_id = vRP.getUserId(source)
		if user_id ~= nil then
			vRP.openMenu(source,announce_menu)
        end
    end

    local function advert_leave(source,area)
		vRP.closeMenu(source)
    end

    vRPclient.addMarker(source,{-601.19915771484,-930.84045410156,22.864507675171,0.7,0.7,0.5,0,125,255,125,150})
    vRP.setArea(source,"vRP:advert",-601.19915771484,-930.84045410156,23.864507675171,1,1.5,advert_enter,advert_leave)
end

AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    build_news_checkpoint(source)
  end
end)
