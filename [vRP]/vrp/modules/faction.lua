...
local function ch_leaveGroup(player,choice)
	...
	if user_id ~= nil then
		if(vRP.hasUserFaction(user_id))then
			TriggerClientEvent("toasty:Notify", player, {type="error", title="Paraseste Factiunea", message="Trebuie sa vorbesti cu liderul tau pentru a parasi factiunea."})
				...
				if(theTarget)then
					...
					vRP.prompt(player,"Factiune: ","",function(player,faction)
						...
						if(faction ~= "") and (faction ~= nil)then
							...
							if(faction == "Politia Romana" or faction == "S.I.A.S" or faction == "Smurd" or faction == "SRI" or faction == "Los Vagos" or faction == "Bloods" or faction == "Crips" or faction == "Ballas" or faction == "Clanu` Tiganilor")then
