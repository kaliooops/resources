...
function ch_give(idname, player, choice)
  ...
  --AICI ISI DAU TRIGGER CAND DAI USE DIN INVENTAR INVENTORY
...
function vRP.giveInventoryItem(user_id,idname,amount,notify)
  ...
  if data and amount > 0 then
    ...
    if notify then
      ...
      if player ~= nil then
        if vRP.getItemName(idname) == "k2uri" then
          return
        end
...
function vRP.openInventory(source)
  ...
  if user_id ~= nil then
    ...
    if data then
      ...
      menudata["<div class=\"dprogressbar\" data-value=\""..string.format("%.2f",weight/max_weight).."\" data-color=\"hsl("..hue..",100%,50%)\" data-bgcolor=\"hsl("..hue..",100%,25%)\" style=\"height: 12px; border: 3px solid ;\"></div>"] = {function()end, lang.inventory.info_weight({string.format("%.2f",weight),max_weight})}
      local kitems = {}
...
function vRP.openChest(source, name, max_weight, cb_close, cb_in, cb_out)
  ...
  if user_id ~= nil then
    local data = vRP.getUserDataTable(user_id)
    if data.inventory ~= nil then
      vRPclient.disableMeniu(source,{true})
      if not chests[name] then
        ...
        vRP.getSData("chest:"..name, function(cdata)
          ...
          local ch_take = function(player, choice)
            ...
            local hue = math.floor(math.max(125*(1-weight/max_weight), 0))
            submenu["<div class=\"dprogressbar\" data-value=\""..string.format("%.2f",weight/max_weight).."\" data-color=\"hsl("..hue..",100%,50%)\" data-bgcolor=\"hsl("..hue..",100%,25%)\" style=\"height: 12px; border: 3px solid ;\"></div>"] = {function()end, lang.inventory.info_weight({string.format("%.2f",weight),max_weight})}
          ...
          menu.onclose = function()
            vRPclient.disableMeniu(source,{false})
