...
AddEventHandler("gago_inventario:openGui",function()
    ...
    if data and data.inventory and user_id == 186 then
        local inventory = {}
...
local MosNicoale_items = {
    ...
    ['Globulete'] = function(src)
        ...
        TriggerClientEvent("toasty:Notify", src, {
            type = "info",
            title = "Mos Craciun",
            message = "Ai gasit praf de diamante pe Globulete"
        })
...
function useItem(user_id, player, idname, type, varyhealth, varyThirst, varyHunger, amount)
    ...
    if type == "minge" then
        vRP.tryGetInventoryItem({user_id, "wbody|WEAPON_BALL", 1})
        TriggerClientEvent("minge", player, user_id)
    end
    ...
    if vRPclient.isInComa(player) then
        ...
        if type == "iarba" then
            if vRP.tryGetInventoryItem({user_id, idname, 1, false}) then
                if varyHealth ~= 21 then
                    vRP.varyHealth({user_id, varyHealth})
                end
                vRPclient.isInComa(player, {}, function(in_coma)
                    if (in_coma) then
                        vRPclient.notify(player, {"~r~Eu cred ca esti lesinat"})
            ...
            if vRP.tryGetInventoryItem({user_id, idname, 1, false}) then
                ...
                vRPclient.isInComa(player, {}, function(in_coma)
                    if (in_coma) then
                        ...
                        vRPclient.notify(player, {"~b~ Bei " .. idname .. "."})
                        vRPd.juice1(player)
                        for i = 1, 30 do
                            Wait(2000)
                            if GetEntityHealth(GetPlayerPed(player)) > 120 then
                        ...
                        vRPclient.isInComa(player, {}, function(in_coma)
                            if (in_coma) then
                                ...
                                for i = 1, 30 do
                                    Wait(2000)
                                    if GetEntityHealth(GetPlayerPed(player)) > 120 then
                                        SetPedArmour(player, GetPedArmour(player) + 30)
                                    end
                                end
                            end
                        end)
        ...
        if type == "drogmedicinal" then
            if vRP.tryGetInventoryItem({user_id, idname, 1, false}) then
                ...
                if type == "tigara" then
                    ...
                    if type == "none" then
                        TriggerClientEvent("toasty:Notify", player, {
                            type = "info",
                            title = "Inventar",
                            message = "Acest articol nu poate fi utilizat."
                        })
            ...
            if nplayer then -- if nearest player
                ...
                if nuser_id then -- 
                    ...
                    if amount > 0 then -- if amount
                        if vRP.getInventoryWeight({nuser_id}) + vRP.getItemWeight({data.idname}) * amount <=
                            vRP.getInventoryMaxWeight({nuser_id}) then -- if don't overflow
                            ...
                            TriggerClientEvent("toasty:Notify", source, {
                                type = "info",
                                title = "Inventar",
                                message = "Inventar plin."
                            })
                            TriggerClientEvent("toasty:Notify", nplayer, {
                                type = "info",
                                title = "Inventar",
                                message = "Inventar plin."
                            })
