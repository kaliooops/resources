  ...
  if vRP.hasPermission({user_id, "#"..cfg.list[id].key..".>0"}) or vRP.hasPermission({user_id,cfg.list[id].permission}) then
    ...
    SetTimeout(4000, function()
      ...
      if cfg.list[id].locked then
        vRPclient.notify(player, {"Ușa este închisă cu Cheia "..cfg.list[id].name})
      else
        vRPclient.notify(player, {"Ușa se deschide cu Cheia "..cfg.list[id].name})
      end
    end)
  else
    vRPclient.notify(player, {"Lipseste Cheia Usii "..cfg.list[id].name})
  end
end)
