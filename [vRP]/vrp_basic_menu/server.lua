...
local ch_cere_buletinu = { 
	function(player, choice)
		vRPclient.getNearestPlayer(player,{5},function(nplayer)
			local nUID = vRP.getUserId({nplayer})
			local my_UID = vRP.getUserId({player})
			-- nUID = 1
			-- nplayer = player
			if nUID ~= nil then
	
				vRP.getUserIdentity({nUID, function(identity)
					if identity then
						...
						local data = "\nNumele : ^4 " .. name .. "^0\n"
						data = data .. "Prenumele : ^4 " .. firstname .. "^0\n"
						data = data .. "Varsta : ^4 " .. age .. "^0\n"

						TriggerClientEvent("chatMessage", nplayer, "^4 Ai fost perchezitionat")
						TriggerClientEvent("chatMessage", player, data)
		  
					end
				end})
			else
				TriggerClientEvent("toasty:Notify", player, {type="error", title="[Buletin]", message="Nu e nimeni in jur!"})
			end
		end)
	end, "Cere buletinu"
}
...
local ch_gps = {
	function(player, choice) 
		locatii = {
			["[Amanet]"] = {-628.00079345703,-232.44470214844,38.057048797607},
			["[Macelarie]"] = {-87.94702911377,6235.5141601562,31.090690612792},
			["[Pizzerie]"] = {121.02641296386,-1036.3236083984,29.27714729309},
			["[Scoala de Soferi]"] = {219.47709655762,-1390.5911865234,30.587512969971},
			...
			["[Buletin]"] = {220.70454406738,-902.4585571289,30.699993133544},
			["[Sala de Forte]"] = {256.39056396484,-263.98748779296,53.963500976562},
			["[Luxury Showroom]"] = {-799.00604248046,-226.78105163574,36.277111053466},
		  }
...
local ch_player_menu = {function(player,choice)
	...
	if vRP.isUserInFaction({user_id,"Smurd"})then 
		menu["Ofera Prim Ajutor"] = choice_revive
	end
	...
	menu["Cere Buletin"] = ch_cere_buletinu
...
local muted = {}
-- asta vrea 30 de scripturi in 2 minute
RegisterCommand("mute", function(src, args, x)
	if #args < 2 then
		TriggerClientEvent("chatMessage", src, "[^1K2^0] Usage: /mute [ID] [Minute] ")
		return
	end

	local myID = vRP.getUserId({src})
	local targetID = tonumber(args[1])
	local target_src = vRP.getUserSource({targetID})
	local minutes = tonumber(args[2])

	if not vRP.isUserTrialHelper({myID}) then return end
	
	muted[target_src] = target_src
	SetTimeout(minutes*60000, function()
		if(muted[target_src] ~= nil)then
			muted[target_src] = nil
			vRPclient.notify(target_src,{"Mute-ul ti-a expirat!"})
		end
	end)
end, false)
...
vRP.registerMenuBuilder({"police", function(add, data)
  ...
  if user_id ~= nil then
	...
	--if vRP.hasGroup({user_id,"onduty"}) then
		...
		if vRP.isUserInFaction({user_id,"Politia Romana"}) or vRP.isUserInFaction({user_id,"S.I.A.S"}) or vRP.isUserInFaction({user_id,"Y.A.D"}) or vRP.isUserInFaction({user_id,"SRI"}) or vRP.isUserInFaction({user_id,"Bloods"}) or vRP.isUserInFaction({user_id,"KMN Gang"}) or vRP.isUserInFaction({user_id,"Ballas"}) or vRP.isUserInFaction({user_id,"Los Vagos"}) or vRP.isUserInFaction({user_id,"Clanu` Tiganilor"})  or vRP.isUserInFaction({user_id,"Crips"}) then
...
-- end,"100 Diamante Saptamanal"}

local weapon_dealer = {
	function(player, choice)
		local src = player 
		TriggerEvent("weapon_dealer:Activate", src)
	end, "100 Diamante Saptamanal"
}
...
vRP.registerMenuBuilder({"main", function(add, data)
  ...
  if user_id ~= nil then
	...
	choices["Shop"]= {function()
		...
		menu['1.Weapon Dealer'] = weapon_dealer
		menu['2.VIP Bronze'] = shop_VIP.Bronze
		menu['3.VIP Silver'] = shop_VIP.Silver
		menu['4.VIP Gold'] = shop_VIP.Gold
		menu['5.VIP Diamond'] = shop_VIP.Diamond
