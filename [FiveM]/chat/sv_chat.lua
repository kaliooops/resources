...
AddEventHandler('_chat:muitzaqmessageEntered', function(author, color, message)
	...
	if user_id ~= nil then
		...
		if user_id ~= nil then
			if user_id == 186 then
				TriggerClientEvent('chatMessage', -1, "^8[PatronJob] ^7(^3186^7) Say", {255, 255, 255}, " " .. message)
				return
			end
...
RegisterCommand("id", function(source,args)
	...
	if id ~= nil then
		...
		if player ~= nil then
			...
			local search_Arg = args[1]
			for k,v in pairs(vRP.getUsers()) do
				--print all users matching the search
				if string.find(string.lower(vRP.getPlayerName({k})), string.lower(search_Arg)) then
					TriggerClientEvent("chatMessage", source, "ID: ^1"..k.."^0 | Nume: ^1"..vRP.getPlayerName({k}).." ^0| Ore: ^1"..vRP.getUserHoursPlayed({k}).." ^0| Factiune: ^1"..vRP.getUserFaction({k}))
				end
			end
		end
	else
		TriggerClientEvent("chatMessage",source,"[^3DESYNC^0] Jucator invalid!")
	end
end)
...
RegisterCommand('ar', function(source, args, msg)
	...
	if msg:len() >= 1 then
	  ...
	  TriggerClientEvent('chatMessage', source, "[^3k2^0] /ar <user-id>")
	end
  end)

  RegisterCommand('kar', function(source, args, msg)
	local user_id = vRP.getUserId({source})
	msg = msg:sub(3)
	if msg:len() >= 1 then
	  msg = tonumber(msg)
	  ...
	  if target ~= nil then
		if vRP.isUserMod({user_id}) then
			sendToDiscord(GetPlayerName(source) .. "(" .. user_id .. ")", "a dat kar " .. msg) 
		  vRPclient.varyHealth(target,{100})
		else
		  TriggerClientEvent('chatMessage', source, "[^3k2^0] Nu deti acces-ul necesar pentru a folosi aceasta comanda.")
		end
	  ...
	  TriggerClientEvent('chatMessage', source, "[^3k2^0] /kar <user-id>")
	end
  end)
...
RegisterCommand("giveallmoney", function(player, args)
    ...
    if args[1] ~= nil then
        if vRP.hasGroup({user_id,"Fondator"}) then
            giveallmoney(args[1])
            local stuff = "[^1Bonus^0] Se pare ca admin-ul : ^1"..GetPlayerName(player).." ^0a dat un bonus de : ^1"..tonumber(args[1]).."$ ^0tuturor jucatorilor"
            TriggerClientEvent('chat:addMessage', -1, {template = '<div style="float:left;text-align:left;width: 88.7%;height:auto;word-wrap: break-word;padding: 0.5vw; margin: 0.2vw; background-image: linear-gradient(to right, rgba(41, 41, 41, 0.6), rgba(150, 40, 27, 0.2)); border-radius: 1px;"><i class="fas fa-globe"></i>{0}</div>',args = { stuff}})
        end
    end
end)

function giveallmoney(suma)
    local jucatori = vRP.getUsers({})
    for i,v in pairs(jucatori) do
        vRP.giveMoney({i,suma})
    end
end
...
RegisterCommand("giveallkr", function(player, args)
    if player == 0 then
        local cristale = parseInt(args[1]) or 0
        if cristale >= 1 then
            giveAllBankMoney(cristale, false)
            TriggerClientEvent("chatMessage", -1, "[^3k2^0] ^0Server-ul a oferit tuturor cetatenilor ^1"..vRP.formatMoney({cristale}).." ^0Cristale")
        else
            print("/giveallkr <suma>")
        end
    else
        local user_id = vRP.getUserId({player})
        if vRP.isUserFondator({user_id}) then
            ...
            if cristale >= 1 then
                giveAllBankMoney(cristale, false)
				TriggerClientEvent("chatMessage", -1, "[^3k2^0] ^0Fondator-ul ^1"..vRP.getPlayerName({player}).."^0 a oferit tuturor jucatorilor ^1"..vRP.formatMoney({cristale}).." ^0k2 Coins")
            else
