...
function SetGridTargets(pos, reset) -- Used to set the players voice targets depending on where they are in the map
	...
	if not playerData then
		playerData  = {
			...
			callSpeaker = false,
			speakerTargets = {},
			radioName = GetPlayerName(PlayerId())
		}
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if initialised then
			...
			if IsControlJustPressed(344, mumbleConfig.controls.proximity.key) then
				...
				if mumbleConfig.controls.speaker.key ~= mumbleConfig.controls.proximity.key or mumbleConfig.controls.speaker.secondary == nil then
					...
					secondaryPressed = IsControlPressed(344, mumbleConfig.controls.speaker.secondary) and (playerData.call > 0)
				end
				...
				if not mumbleConfig.controls.radio.pressed then
					if IsControlJustPressed(0, mumbleConfig.controls.radio.key) then
						if playerData.radio > 0 then
							...
							Citizen.CreateThread(function()
								while IsControlPressed(344, mumbleConfig.controls.radio.key) do
									Citizen.Wait(0)
								end
			...
			if mumbleConfig.callSpeakerEnabled then
				...
				if mumbleConfig.controls.speaker.secondary ~= nil then
					secondaryPressed = IsControlPressed(344, mumbleConfig.controls.speaker.secondary)
				else
