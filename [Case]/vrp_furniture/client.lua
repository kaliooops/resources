vRPfurnitureC = {}
Tunnel.bindInterface("vRP_furniture",vRPfurnitureC)
Proxy.addInterface("vRP_furniture",vRPfurnitureC)
...
furni.init = function()
  SendNUIMessage({
    type              = "init",
    onOpen            = "http://vrp_furniture/DoOpen",
    onClose           = "http://vrp_furniture/DoClose",
    onCancel          = "http://vrp_furniture/DoCancel",
    onSelect          = "http://vrp_furniture/DoSelect",
    onPlace           = "http://vrp_furniture/DoPlace",
    onStartAim        = "http://vrp_furniture/DoStartAim",
    onStopAim         = "http://vrp_furniture/DoStopAim",
    onReposition      = "http://vrp_furniture/DoReposition",
    onRepositionStart = "http://vrp_furniture/DoRepositionStart",
    onTransform       = "http://vrp_furniture/DoTransform",
    onEdit            = "http://vrp_furniture/DoEdit",
    onRemove          = "http://vrp_furniture/DoRemove",
    onExit            = "http://vrp_furniture/DoExit",
    items             = furni.objects,
    top               = "300px",
    left              = "20px",
  })
            ...
            if ePos.x ~= 0.0 then
              ...
              if not furni.antiGrab[model] then
                if wasPressing and keyHeldFor >= 500 then
                  Utils.drawText3D(pos,"~r~Elibereaza [~s~CLICK STANGA~r~] Ca Sa Schimbi [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~r~]~s~",1,7)
                else
                  Utils.drawText3D(pos,"~y~Tine Apasat [~s~CLICK STANGA~y~] Ca Sa Schimbi [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~]~s~",1,7)
                end
        ...
        elseif furni.doingRemoval then
          ...
          if ent and ent ~= -1 and ent ~= 0 then
            ...
            if ePos and ePos.x and ePos.x ~= 0 and ePos.x ~= 0.0 then
              local model = GetEntityModel(ent)
              if not furni.antiGrab[model] then
                Utils.drawText3D(pos,"~y~Tine Apasat [~s~CLICK STANGA~y~] Sa Vinzi [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~] [~s~"..(hashtable[model] and "$"..math.floor(hashtable[model].price*(0.5)) or "Unknown").."~y~]~s~",1,7)
                if IsDisabledControlPressed(0, Controls["Grab"]) or IsControlPressed(0, Controls["Grab"]) and not furni.focused then
                  keyHeldFor = keyHeldFor + (GetGameTimer() - lastFrame)
                  if keyHeldFor >= 500 then
                    keyHeldFor = 0
...
furni.doPlace = function(...)
  ...
  if furni.inHouse then
    ...
    if furni.wasEditing then
      ...
      vRPfurnitureB.GetPlayerFullCash({}, function(money)
        if tonumber(money) then 
          if tonumber(money) >= tonumber(item.price) then
            TriggerServerEvent('furni:PlaceFurniture', furni.inHouse,item,pos,rot,furni.object)
          else
            Utils.showNotification("[~y~Star~w~] ~r~Nu ai destui bani pentru acest obiect!")
            SetEntityAsMissionEntity(furni.object,true,true)
            DeleteObject(furni.object)
          end
        end
...
furni.openCommand = function(s,args) 
  if furni.inHouse then
    ...
    Utils.showNotification("[~y~Star~w~] ~r~Trebuie sa fi intr-o casa!")
  end
end
