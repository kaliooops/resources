...
RegisterNUICallback('crewPhone_uberTrigger', function()
    if uberBasladi then
        exports['okokNotify']:Alert('Uber Eats', 'Ai anulat cautarea', 5000, 'info')
		uberBasladi = false
		paketNoktasi = true	
        RemoveBlip(uberBlip)
		...
		exports['okokNotify']:Alert('Uber Eats', 'Cauti livrare [Activ]', 5000, 'info')
		uberBasladi = true
        paketNoktasi = false
        SendNUIMessage({event = 'updateUber', status = uberBasladi})
    end
	...
	while true do
		...
		if uberBasladi and not paketNoktasi then
			...
			if uberBasladi then
				...
				exports['okokNotify']:Alert('Uber Eats', 'Livrare noua: ' .. Config.uberItems[uberChoosenItem]["name"] .. ' marked on map', 5000, 'info')
				paketNoktasi = true
...
function rewarduber()
	...
	exports['okokNotify']:Alert('Uber Eats', 'Ai livrat cu succes si ai primit' .. math.Round(price), 5000, 'info')
	if npcEvdemi == 1 then
		local bahsis = math.random(3,10)
		local toplamPara = price + bahsis
		TriggerServerEvent('esx_uber:pay', toplamPara)
		        exports['okokNotify']:Alert('Uber Eats', 'Ai primit bacsis ' .. bahsis, 5000, 'info')

	elseif npcEvdemi ~= 1 then
		TriggerServerEvent('esx_uber:pay', price)
		exports['okokNotify']:Alert('Uber Eats', 'Ai livrat comanda si ai primit '..price..'$', 5000, 'success')
    end
    SendNUIMessage({event = 'updateSiparisStatus', status = false})
end
	...
	while true do
		...
		if uberBasladi and uberChoosenAdress ~= nil then
			if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.uberDelivery[uberChoosenAdress]["x"], Config.uberDelivery[uberChoosenAdress]["y"], Config.uberDelivery[uberChoosenAdress]["z"], true ) < 1.2 and paketNoktasi and not kapiCalindi then
				Draw3DText2(Config.uberDelivery[uberChoosenAdress]["x"], Config.uberDelivery[uberChoosenAdress]["y"], Config.uberDelivery[uberChoosenAdress]["z"] + 0.3, tostring("~w~~g~[E]~w~ Bate la usa"))
			
				if(IsControlJustPressed(1, Keys["E"])) then
					TriggerServerCallback('crew-phone:item-check', function(qtty)
						if qtty > 0 then
							...
							exports['okokNotify']:Alert('Uber Eats', 'Nu ai ' .. Config.uberItems[uberChoosenItem]["name"], 5000, 'info')
						end
