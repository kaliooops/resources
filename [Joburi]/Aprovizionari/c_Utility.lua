...
Dresser = { 
    dressed = false,
    location = { x = 868.08, y = -1629.46, z = 30.25 },
    cleanClothes = {}
...
function dressUP() 
    ...
    if not Dresser.dressed then
        ...
        if GetEntityModel(PlayerPedId()) == -1667301416 then
            SetPedPropIndex(ped, 0, 107, 2, false) -- hat
            SetPedComponentVariation(ped, 11, 398, 0, 0) -- jacket
            SetPedComponentVariation(ped, 3, 14, 0, 0) -- armsgloves
            SetPedComponentVariation(ped, 4, 122, 0, 0) -- pants
            SetPedComponentVariation(ped, 6, 94, 0, 0) -- shoes
            SetPedComponentVariation(ped, 1, 0, 0,  0) -- mask    
            return
        end


        SetPedPropIndex(ped, 0, 83, 6, false) -- hat
        SetPedComponentVariation(ped, 11, 370, 8, 0) -- jacket
        SetPedComponentVariation(ped, 3, 62, 0, 0) -- armsgloves
        SetPedComponentVariation(ped, 4, 8, 4, 0) -- pants
        SetPedComponentVariation(ped, 6, 4, 11, 0) -- shoes
        SetPedComponentVariation(ped, 1, 0, 0, 0) -- mask
    else
