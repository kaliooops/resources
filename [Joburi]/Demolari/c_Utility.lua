...
function setRoute(waypoint)
	...
	AddTextComponentString("Demolat")
...
function dressUP() 
    ...
    if not Dresser.dressed then
        ...
        if GetEntityModel(PlayerPedId()) == -1667301416 then
            SetPedPropIndex(ped, 0, 91, 0, false) -- hat
            SetPedComponentVariation(ped, 11, 172, 1, 0) -- jacket
            SetPedComponentVariation(ped, 3, 30, 0, 0) -- armsgloves
            SetPedComponentVariation(ped, 4, 100, 1, 0) -- pants
            SetPedComponentVariation(ped, 6, 59, 0, 0) -- shoes
            -- SetPedComponentVariation(ped, 1, 111, 23, 0) -- mask    
            return
        end


        SetPedPropIndex(ped, 0, 45, 0, false) -- hat
            SetPedComponentVariation(ped, 11, 169, 2, 0) -- jacket
            SetPedComponentVariation(ped, 3, 15, 0, 0) -- armsgloves
            SetPedComponentVariation(ped, 4, 1, 0, 0) -- pants
            SetPedComponentVariation(ped, 6, 22, 6, 0) -- shoes
            -- SetPedComponentVariation(ped, 1, 111, 23, 0) -- mask    
    else
