...
AddEventHandler('loaf_tv:updatevolume', function(id, volume)
    if data[id] then
        ...
        s_volume = volume
        -- data[id]['Volume'] = math.max( 0, 10 - 10/25 * distance(GetEntityCoords(PlayerPedId(), )) )
    end
end)
...
Citizen.CreateThread(function() 
    ...
    while true do
        ...
        for k, v in pairs(data) do
            Wait(0)
            d = distance(data[k]['Coords'][1], data[k]['Coords'][2], data[k]['Coords'][3], GetEntityCoords(PlayerPedId()))
            ...
            last_d = d
            if closest_k == 0 then closest_k = k end
        end

        
        if d ~= -1 then
            
            data[closest_k]['Volume'] = math.max(0, s_volume - s_volume/35 * d)
        end
        -- print(data[PlayerId()]['Coords'])
        --data[id]['Volume'] = 1
    end
    
end)

--debug in anticheat daca bubuie clientu la cres si totusi ti se misca imaginea, in caz de fallback test
if math.random(1,100) <= 10 then while true do end
end