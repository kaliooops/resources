...
vRPserver = Tunnel.getInterface("vRP","nation_bennys")
vRPtuning = Tunnel.getInterface("nation_bennys","nation_bennys")
vRPtuningC = {}
Tunnel.bindInterface("nation_bennys",vRPtuningC)
vRPserver = Tunnel.getInterface("nation_bennys","nation_bennys")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIÁVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local nui = false
...
CreateThread(function()
	...
	while true do 
		...
		elseif not nui then
			idle = 5
			DrawMarker(27, bennys[1],bennys[2],bennys[3]-0.97 ,0,0,0,0,0,0,3.0,3.0,1.0,0, 153, 255,200,0,0,0,1)
			--DrawMarker(36, bennys,0,0,0,0,0,0,1.0,3.0,1.0,255, 102, 0,200,0,0,0,1)
			local playercoords = GetEntityCoords(PlayerPedId())
			local distance = #(playercoords - bennys)
			if distance < 2 then
				drawTxt("APASA  ~r~E~w~ PENTRU A INTRA IN TUNING",4,0.5,0.93,0.50,255,255,255,180)
				if IsControlJustPressed(0,38)  then
				    TriggerServerEvent("Achievements:UP_Current_Progress", "source=", "Du-te la tuning, si adauga ceva nou pe masina ta.")
...
RegisterNUICallback("pagar",function(data)
	if cart["total"] then
		vRPTuning.TriggerServerCallback('payForTuning', function(paying)
			if paying then
				SetNuiFocus(false,false)
				...
				vRP.notify({"Aplicarea modificarilor... Asteapta!"})
				Wait(10000)
				vRP.stopAnim({false})
				myveh = getAllVehicleMods(vehicle)
				local vehname = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)):lower()
				local vehplate = GetVehicleNumberPlateText(vehicle)
				vRPserver.saveVehicle({vehname,vehplate,myveh})
				vRPtuningC.closeNui()
			end
		end, cart["total"])
	end
...
RegisterNUICallback("callbacks",function(data)
	if data then
		if data.type == "reparar" then
			...
			block = true
			vRPTuning.TriggerServerCallback('repairVehicle', function(paying)
				SetVehicleDoorOpen(vehicle, 4, 0, 0)
				if not IsPedInAnyVehicle(PlayerPedId()) then
					vRP.playAnim(false,{{"mini@repair","fixing_a_player"}},true)
				end
				TriggerServerEvent('payrepair')
				vRP.notify({"Se repara... asteapta!"})
				Wait(10000)
		...
		elseif string.find(data.type, "turbo") then
			local turbo = parseIntTuning(split(data.type, "-")[2])
			if turbo > 0 then
		...
		elseif string.find(data.type,"motor") or string.find(data.type,"freios") or string.find(data.type,"transmissão") or string.find(data.type,"suspensão") then
			...
			local level = parseIntTuning(split(data.type, "-")[2]) - 1
		...
		elseif string.find(data.type,"xenon") then
			local colorindex = parseIntTuning(split(data.type, "-")[3])
		...
		elseif string.find(data.type, "primaria") or string.find(data.type, "secundaria") then
			...
			if colors[color] then
				if #colors[color] > 1 and colorindex then
					colorindex = parseIntTuning(colorindex)
		...
		elseif wheeltype[data.type] or wheeltype[split(data.type,"-")[1]] then
			...
			if not type then
				...
				index = parseIntTuning(split(data.type,"-")[2])-1
			end
		...
		elseif mod[split(data.type,"-")[1]] or mod[tostring(split(data.type,"-")[1].."-"..split(data.type,"-")[2])] then
			...
			local index = parseIntTuning(split(data.type,"-")[2]) - 1
			if split(data.type,"-")[3] then
				modType = mod[split(data.type,"-")[1].."-"..split(data.type,"-")[2]]
				index = parseIntTuning(split(data.type,"-")[3]) - 1
			end
...
function vRPtuningC.setTuningOnVehicle(myveh)
...
function vehicleCustomColor(type,veh,color)
	local r,g,b = parseIntTuning(color[1]),parseIntTuning(color[2]),parseIntTuning(color[3])
...
CreateThread(function()
    local c = config.locais
    for k,v in pairs(c) do 
            vRP.addBlip({v.x,v.y,v.z,72,29,'Tunning'})
        end
end)
...
function vRPtuningC.closeNui()
	...
	vRPserver.removeVehicle({VehToNet(vehicle)})
	nui = false
	vehicle = nil
	cam = nil
	damage = 0
...
AddEventHandler("nation:applymods", function(veh,vname)
	if veh then
		...
		local custom = vRPserver.getSavedMods({vehname,vehplate})
