...
for (let i = 0, length = GetNumResourceMetadata("radio", "supersede_radio"); i < length; i++) {
    ...
    if (!availableRadios.includes(radio)) {
        console.error(`radio: ${radio} este invalid.`);
        continue;
    }
    ...
    try {
        ...
        if (data !== null) {
            ...
            console.error(`radio: Lipsesc date pentru: ${radio}.`);
        }
    } catch (e) {
        console.error(e);
    }
...
RegisterNuiCallbackType("radio_ready");
on("__cfx_nui:radio_ready", (data, cb) => {
